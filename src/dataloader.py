...
def Dataloaders(search_strategy,config,arg):
    ...
    if search_strategy in ['None','sync','random'] :
        ...
        return split_for_nas(train_dataset,valid_dataset,config,arg,
                            split_for_train = config.train.split_for_train,
                            split_for_valid = config.train.split_for_archvalid)
        
def split_for_nas(train_dataset,valid_dataset,config,arg,split_for_train=2,split_for_valid=1):
    ...
    logger.info(train_iters)
    logger.info(valid_iters)
    ...
    if arg.distributed:
        return train_queue ,arch_queue, valid_queue ,train_dist_sampler
    else:
