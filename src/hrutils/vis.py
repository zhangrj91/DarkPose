...
mpii_part_labels = ['head', 'upper neck', 'right shoulder', 'right elbow', 'right wrist',
                   'left shoulder', 'left elbow', 'left wrist', 'right pelvis',
                   'right knee', 'right ankle', 'left pelvis', 'left knee', 'left ankle',
                   'background']
mpii_part_idx = {
    b: a for a, b in enumerate(mpii_part_labels)
}
mpii_part_orders = [
    ('head', 'upper neck'),
    ('upper neck', 'right shoulder'),
    ('upper neck', 'left shoulder'),
    ('right shoulder', 'right elbow'),
    ('right elbow', 'right wrist'),
    ('left shoulder', 'left elbow'),
    ('left elbow', 'left wrist'),
    ('right shoulder', 'right pelvis'),
    ('left shoulder', 'left pelvis'),
    ('right pelvis', 'right knee'),
    ('right knee', 'right ankle'),
    ('left pelvis', 'left knee'),
...
VIS_CONFIG = {
    ...
    'mpii': {
        'part_labels': mpii_part_labels,
        'part_idx': mpii_part_idx,
        'part_orders': mpii_part_orders
    }
}
...
def save_valid_image(image, joints, file_name, dataset='COCO'):
    ...
    if dataset == 'coco':
        ...
        color = np.random.randint(0, 255, size=3)
        color = [int(i) for i in color]
        add_joints(image, joints, color, dataset=dataset)
