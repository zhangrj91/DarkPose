...
from src.hrutils.vis import save_valid_image
...
def inference(model ,input, affine_matrix_inv, info , upsample, flip_test=False ,
    ...
    heatmap_dt = heatmap_dt[:,-1]
    # heatmap_dt = (heatmap_dt[:,-1] + heatmap_dt[:,-2] + heatmap_dt[:,-3])/3
    z = heatmap_dt.clone()
    # flip-test
    if flip_test:
        input_filp = input.flip([3])
        heatmap = model(input_filp)
        heatmap_dt_flip = heatmap[:,-1]
...
##################### evaluate #####################
def evaluate( model , dataloader , config, output_dir, imageId, with_mask = False, use_refine = False ,boosting = None):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for id ,(input,image_id ,score ,affine_matrix_inv, bbox, info) in enumerate(dataloader):
            ...
            if imageId != '':
                for i in range(len(input)):
                    if int(imageId) == image_id[i]:
                        pred = coco_like_to_mpii_format(all_preds)
                        img = cv2.imread("data/mpii/images/{}.jpg".format(imageId))
                        save_valid_image(img, pred[idx - num_images + i], os.path.join(output_dir, 'image_{}.jpg'.format(imageId)), dataset="mpii")
    ...
    if dataset_name =='coco':
        ...
        if config.test.dataset_name == 'test':
            dt_path = os.path.join(output_dir, 'dt_test.json')
...
def mpii_eval(config, dt, output_dir):
    ...
    # convert 0-based index to 1-based index
        ...
        if output_dir:
            if config.test.outputdir == 'pred_val.mat':
                pred_file = os.path.join(output_dir, 'pred_val.mat')
            else:
                pred_file = os.path.join(output_dir, 'pred.mat')
            savemat(pred_file, mdict={'pred': preds})

        if 'test' in config.test.dataset_name:
           return {'Null': 0.0}, 0.0
